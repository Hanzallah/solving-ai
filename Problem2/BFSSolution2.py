'''
This code solves:
---There are 6 cannibals, 6 missionaries, and a boat holding 4---
'''
'''
...
def getAllPossibleNextNodes(node):
    ...
    # The boat capacity is 4 for this one beacuse of that there is only 12 different possibilities for each coast for the boat traversal
    possibleNodes = []
    if rowBoatLocation == "west":
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]-0, nodeArray.list[1]-4, 1, nodeArray.list[3]+0, nodeArray.list[4]+4])):
            possibleNodes.append(Node([nodeArray.list[0]-0, nodeArray.list[1]-4, 1, nodeArray.list[3]+0, nodeArray.list[4]+4]))
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]-2, nodeArray.list[1]-2, 1, nodeArray.list[3]+2, nodeArray.list[4]+2])):
            possibleNodes.append(Node([nodeArray.list[0]-2, nodeArray.list[1]-2, 1, nodeArray.list[3]+2, nodeArray.list[4]+2]))
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]-3, nodeArray.list[1]-1, 1, nodeArray.list[3]+3, nodeArray.list[4]+1])):
            possibleNodes.append(Node([nodeArray.list[0]-3, nodeArray.list[1]-1, 1, nodeArray.list[3]+3, nodeArray.list[4]+1]))
        if isValidNode(nodeArray, Node([nodeArray.list[0]-4, nodeArray.list[1]-0, 1, nodeArray.list[3]+4, nodeArray.list[4]+0])):
            possibleNodes.append(Node([nodeArray.list[0]-4, nodeArray.list[1]-0, 1, nodeArray.list[3]+4, nodeArray.list[4]+0]))
    else:
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]+0, nodeArray.list[1]+4, 0, nodeArray.list[3]-0, nodeArray.list[4]-4])):
            possibleNodes.append(Node([nodeArray.list[0]+0, nodeArray.list[1]+4, 0, nodeArray.list[3]-0, nodeArray.list[4]-4]))
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]+2, nodeArray.list[1]+2, 0, nodeArray.list[3]-2, nodeArray.list[4]-2])):
            possibleNodes.append(Node([nodeArray.list[0]+2, nodeArray.list[1]+2, 0, nodeArray.list[3]-2, nodeArray.list[4]-2]))
        ...
        if isValidNode(nodeArray, Node([nodeArray.list[0]+3, nodeArray.list[1]+1, 0, nodeArray.list[3]-3, nodeArray.list[4]-1])):
            possibleNodes.append(Node([nodeArray.list[0]+3, nodeArray.list[1]+1, 0, nodeArray.list[3]-3, nodeArray.list[4]-1]))
        if isValidNode(nodeArray, Node([nodeArray.list[0]+4, nodeArray.list[1]+0, 0, nodeArray.list[3]-4, nodeArray.list[4]-0])):
            possibleNodes.append(Node([nodeArray.list[0]+4, nodeArray.list[1]+0, 0, nodeArray.list[3]-4, nodeArray.list[4]-0]))
...
'''
def bfs():
    visited.append(Node([6, 6, 0, 0, 0]))  # Root state(Node)
    queue.append(Node([6, 6, 0, 0, 0]))  # Root state(Node)

    while queue:
        s = queue.pop(0)  # Traversing the queue
        finalList.append(s.list)
        if s.list == [0, 0, 1, 6, 6]:  # Finishing condition
            break

        possibleNodes = getAllPossibleNextNodes(s)  # getting all possible next states
        for i in range(0, len(possibleNodes)):
            flag = False
            for j in range(0, len(visited)):  # Checking whether a state is previously visited
                if possibleNodes[i].list == visited[j].list:
                    flag = True
            if flag is False:  #  If it is the first time add it to the queue and mark it as visited
...
for j in range(0, len(finalList)):  # We need to parse all of the tree
    ...
    if finalList[j][2] == 1 and flag is True:  # This checks for West coast transition states
        for i in range(0, len(finalList)):
            ...
            k = None
            if (j+i+1) < len(finalList):  # We finish it if we reach at the end
                k = j+i+1
            else:
                break
            if finalList[k][2] == 0:
        ...
        if len(tempList) == 1:  # If there is only one state then we can just added it since every next states should be connected to it
            ...
            for i in range(0, len(tempList)):  # If there is more than one possible next state we choose a propers one tthat connects to our current state
                ...
                dif2 = result[-1][1] - tempList[i][1]
                if dif1 >= 0 and dif2 >= 0 and (dif1+dif2) <= 4:
        ...
        if len(tempList) == 1:  # If there is only one state then we can just added it since every next states should be connected to it
            ...
            for i in range(0, len(tempList)):  # If there is more than one possible next state we choose a propers one tthat connects to our current state
                ...
                if dif1 >= 0 and dif2 >= 0 and (abs(dif1)+abs(dif2)) <= 4:
                    result.append(tempList[i])
                    flag = True
